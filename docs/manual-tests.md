...
4. **Bugünün blokları (halkaların altı):** "bugünün 5 saatlik blokları" başlığı, 00–24 ekseninde her blok kendi saatinde sabit yükseklikte bir şerit, **içinde token sayısı** (ör. "31M"), aktif blok turuncu ve çalışırken parlar, ince beyaz çizgi şu anki saat. Blokların toplamı `#` çipindeki günlük token'la örtüşmeli; `ccusage claude blocks --json --since <bugün> --offline` ile aynı bloklar (saatler UTC'den yerele çevrilir, blok başlangıcı ccusage kuralıyla tam saate yuvarlanır).
