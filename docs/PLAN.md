...
| Popup yerleşimi | Çentikli ekranda popup satırı (kapak + başlık + sanatçı) **kameranın altındaki şeride** iner: yüzey `notch.height + popupExtraHeight (36)`, içerik `popupTopInset = notch.height` ile başlar ve tüm genişliği kullanır. Önceki yerleşim (kanat hizasında, ortalanmış) uzun başlıkları kameranın arkasına sokuyordu (2026-09-06 kullanıcı raporu). Floating kapsülde inset 0, içerik ortalı. `-debugState popup -debugModule media` uzun başlıkla medya popup'ını gösterir. |
