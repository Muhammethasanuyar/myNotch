...
1. **Açılış animasyonu:** kart açılırken halkalar, başlık, çipler, model satırı ve alt satır sırayla (70 ms arayla) belirir; halka yayları 0'dan süpürülür, model çubuğu paylarına oturur.
2. **Halkalar:** kalın yay kullanılan pay, ince dış yay pencerenin geçen kısmı (30 sn'de bir ilerler), içinde yüzde + "5h"/"7d". Yüzdeler `/usage` ile aynı (2026-09-04: %6 / %52). Claude çalışırken 5h halkası nefes alır gibi parlar. Halkanın üstü çentiğin altına girmez (`expandedTopGap` 8 pt).
3. **Hover = öne çıkma + açıklama:** imleci herhangi bir göstergenin (halka, çip, model satırı, durum noktası, plan çipi, çalışıyor göstergesi) üstüne getir → gösterge %8 büyür ve turuncu parlar, kartın sağ alt bölgesinde siyah bir balon tam cümlelik açıklama gösterir (ör. "5-hour limit — 26% of the allowance used, resets in 4h 26m. Outer arc: 11% of the window has passed."). İmleç çekilince balon kayarak kapanır. Balon tıklamaları yutmaz.
4. **Çipler:** `$` spend (fiyatı bilinmeyen model varsa "—"), `#` tokens, alev = pace ("3K/min"); altlarında tek kelimelik başlık. Değerler değişince rakamlar kayar, ikon zıplar; Claude çalışırken alev titrer. ccusage yoksa tek çip "↓ ccusage / install", açıklaması kurulum komutunu verir.
5. **Durum ve çalışma:** alt satırdaki nokta nabız atar (yeşil taze, turuncu bayat/ağ/rate-limit, kırmızı kimlik sorunu — o zaman yanında kısa komut). Başlık sağında Claude çalışırken proje adı + dalgalanan üç nokta, boştayken küçük sabit nokta. Sağda plan çipi.
6. **Çalışıyor algısı (≤5 sn):** Claude Code'da bir mesaj gönder → panel başlığı "Working · <proje>" olur ve ✳ nabız gibi atar; 10 sn sessizlikten sonra "Idle". Müzik çalmıyorsa compact şeritte ✳ + yüzde görünür; müzik çalarken şerit medyada kalır (öncelik 10 > 5), popup'lar yine gelir.
7. **Kimlik yok:** `security find-generic-password -s "Claude Code-credentials"` boş dönen bir hesapta alt satır "Sign in with `claude` in Terminal to see limits"; `claude` ile giriş yapınca ≤10 sn içinde halkalar dolar (5 sn'lik metadata izleme). Keychain şifresi **sorulmamalı**.
8. **Eşik popup'ı:** 5 saatlik pencere %80'i geçince bir kez "5-hour limit at 8x%" popup'ı; %95'te ikinci; aynı pencerede tekrar yok; pencere sıfırlanınca "5-hour window reset". Debug Preview → `claude` satırı → "Test popup" genel popup yolunu dener.
9. **ccusage yok:** `defaults write com.emre.mynotch ccusagePath /nonexistent` ile bile npx bulunursa çalışır; nvm/npx de yoksa "Cost needs ccusage · brew install ccusage" satırı, halkalar etkilenmez. Geri almak için `defaults delete com.emre.mynotch ccusagePath`.
10. **Ağ yok:** alt satır "Anthropic unreachable · showing last reading", halkalar soluk; 15 dk'dan eski okuma soluk kalır.
11. **Uyku/uyanma:** kapağı kapatıp açınca ilk istek en erken 60 sn sonra; log'da (`log stream --predicate 'subsystem == "com.emre.mynotch"'`) tek poll görünmeli, seri istek yok.
