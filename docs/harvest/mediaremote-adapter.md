...
### 6.1 Cevaplar (2026-09-06, Faz 6 B)

- **S1** — İncelenmedi; plan fork'a bağımlı değil (upstream artefaktları kendimiz derliyoruz). Açık kalır; release öncesi bakım maliyeti sorusu olarak yeniden değerlendirilebilir.
- **S2** — **Kendimiz derledik**, cmake'siz: `scripts/vendor-mediaremote-adapter.sh` `references/ungive/mediaremote-adapter` klonunu SHA `3ac3d4b` ile doğrular, `clang -dynamiclib` (arm64 + x86_64, `lipo`) ile framework'ü, `src/test/*.m` ile test client'ı üretir, ad-hoc imzalar, `Vendor/mediaremote-adapter/` altına yazar (`MANIFEST.txt` sha256'larla). XcodeGen gömer (`embed: true, codeSign: true, link: false`; `.pl` + test client `buildPhase: resources`). `THIRD_PARTY_LICENSES.md` BSD-3 tam metin.
- **S3** — Boşta `stream --debounce=250` (artwork açık): perl **%0,0 CPU / 6,5 MB RSS**, uygulama tarafı %0,0. Adaptör mevcut öğeyi başlangıçtan ≤3 sn içinde bildirdi. Parça değişimi → ilk satır gecikmesi ve artwork gecikmesi henüz ölçülmedi (ölçüm anında yalnızca duraklatılmış bir Edge/YouTube öğesi vardı) — `docs/manual-tests.md` Faz 6.
- **S4** — Çözüldü: `GenericSourceRules.isOwnedByScriptProvider` — `bundleIdentifier` **ya da** `parentApplicationBundleIdentifier` Spotify/Music'e aitse generic sağlayıcı `isRunning() == false` döner, AppleScript sağlayıcısı tek kaynak kalır. Titreme yok çünkü generic hiç event üretmez.
- **S5** — İlk çalıştırmalarda stderr satırı görülmedi. Her stderr satırı `AdapterLine.diagnostic` olarak `Logger(category: "mediaremote-adapter")` info seviyesinde loglanır, stream'i bozmaz; JSON'a çözülemeyen stdout satırı error loglanır ve atlanır.
- **S6** — Release işine kaldı (Developer ID + notarization henüz yok). Ad-hoc imzalı framework'ü `/usr/bin/perl` Debug'da sorunsuz yükledi.
- **§3.10 "Dikkat" notu uygulandı:** açılışta bir kez `pkill -f <gömülü .pl yolu>` (bayraktan bağımsız; stream bu süpürmeyi bekler), `applicationWillTerminate` → `ModuleManager.stopAll()` → SIGTERM + `waitUntilExit`. Öğrenilen: AppKit SIGTERM'i zarif kapanışa çevirmez; `pkill -x MyNotch` sonrası perl hayatta kalıyordu → `DispatchSource.makeSignalSource(SIGTERM)` → `NSApp.terminate`.
