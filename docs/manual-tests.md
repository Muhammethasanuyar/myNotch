...
1. **Halkalar:** "5-hour limit" ve "Weekly limit" yüzdeleri ("21% used") Claude Code'daki `/usage` çıktısıyla aynı olmalı (2026-09-04: %6 / %52 eşleşti). Altında "resets in 4h 35m". Alt satır "Limits updated <1m ago" + sağda plan çipi ("Team") (kimlikteki `subscriptionType`). Her satırın üzerine gelince tooltip açıklar.
2. **Başlık kırılmaz:** "Claude Code" tek satırda kalır; sağdaki durum ("Working · dynamic-notch" / "Idle · <proje>") gerekirse kısalır.
3. **Today satırı:** "10.2M tokens · $27.68" = `ccusage claude daily --json --since <bugün> --offline` toplamı. ccusage'ın çevrimdışı fiyat tablosunda olmayan bir model (2026-09-05: Fable 5.1) 0 $ çıkar; satır "$0.00" yerine **"no price"** der, tooltip hangi model olduğunu söyler.
4. **Models / Model satırı:** iki ve daha çok model varsa "Models ● Opus 5 74% ● Fable 5.1 26%" ve altında iki renkli **parçalı** çubuk (pay çubuğu, kota değil); tek model varsa çubuk çizilmez, "Model Fable 5.1 · all of today's work" yazar. Fiyat yoksa paylar çıktı token'ına göre.
5. **Burn satırı:** aktif 5 saatlik bloğun hızı: "3K tok/min" ve fiyat biliniyorsa "· $32/h · ≈$151 by reset". Blok yoksa satır yok.
