...
| Hover toleransı | Expanded'dan çıkışta 1,5 s gecikme (`NotchViewModel.closeDelay`, Debug Preview'da slider) + `NotchLayout.graceRect`: kart ile 32/28 pt kenar payı, ekran üst kenarına kadar. Süre dolunca `NSEvent.mouseLocation` bölgedeyse yeni bir süre başlar. Görünmez alan **çizilmez** (alfa tabanlı tıklama geçirgenliğini bozar ve tıklamaları yutar); bölge geometrik hesaplanır |
