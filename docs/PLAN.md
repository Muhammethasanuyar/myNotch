...
| `ryoppippi/ccusage` | MIT | JSONL → maliyet/blok hesabı; `--json` çıktısı; `@ccusage/mcp` | **Araç olarak çağır** (Faz 4), hesap mantığını v2'de adapte et |
...
| `farouqaldori/vibe-notch` (eski adı Claude Island) | Klonlarken doğrula | Claude Code CLI oturumlarını notch'tan canlı izleme, araç izinlerini notch'tan onaylama, konuşma geçmişi — Claude modülümüzün gelecek sürümü için fikir kaynağı | Lisansa göre; varsayılan davranış-only |
| `stevemcqueenz/claude-notch-tracker` | Klonlarken doğrula | Claude usage'ı (5 saat / 7 gün / kredi) Keychain veya tarayıcı oturumundan okuyan alternatif yaklaşım | Davranış incele |
| `Lakr233/NotchDrop` | Klonlarken doğrula | Notch'a sürükle-bırak dosya rafı + AirDrop (boring.notch'un Shelf'inin temeli) | Backlog (Faz 6+) |
| `fayazara/macos-app-skills` | Klonlarken doğrula | Claude Code için hazır skill seti: macOS AppKit/SwiftUI desenleri (NSPanel, pencere seviyeleri, çoklu ekran) + `notch-ui` skill'i (CGShieldingWindowLevel'da borderless panel, içbükey "kulaklı" NotchShape Bezier'i) | **Skill olarak kur** (Faz 0.5) |
...
- Hover: `NSTrackingArea` (mouseEntered/Exited) → küçük gecikmeyle (ayarlanabilir, ~0.15 sn) expand; dışarı tıklama / ESC → collapse.
...
- **Resmi limitler (Faz 4'ün ana yolu — codex-island'dan MIT adapte):** Claude Code'un yerelde bıraktığı kimliği **salt-okunur** çöz (`CLAUDE_CODE_OAUTH_TOKEN` → `$CLAUDE_CONFIG_DIR/.credentials.json`, normalde `~/.claude/.credentials.json` → Keychain'deki `Claude Code-credentials` öğesi) ve Anthropic'in kendi usage endpoint'inden 5 saatlik + haftalık pencerelerin **resmi doluluk yüzdesini ve sıfırlanma zamanını** çek. Bu bilgi JSONL'den türetilemez. Dikkat: endpoint belgesiz (kırılabilir) ve agresif rate-limit'li → polling ≥5 dk; hata durumunda son iyi değeri koru. Kaynak: `codex-island/Sources/Usage/UsageFetcher.swift`.
- **Maliyet/token (MVP'de araç, hızlı):** `Process` ile `npx ccusage@latest blocks --json` ve `daily --json` çağır (30–60 sn'de bir + dosya değişiminde). Maliyet hesabını sıfırdan yazma — battle-tested. (codex-island'ın `Sources/Cost/` okuyucuları da MIT alternatif.)
