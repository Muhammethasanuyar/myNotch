...
1. **Halkalar (görsel):** iki halka, içinde yüzde ve "5h"/"7d" rozeti; kalın yay kullanılan pay, **ince dış yay pencerenin ne kadarının geçtiği** (dakikada bir ilerler). Kart açılırken yaylar 0'dan süpürülür. Altında ↺ + kalan süre. Yüzdeler Claude Code'daki `/usage` ile aynı olmalı (2026-09-04: %6 / %52 eşleşti). Halkanın üstü çentiğin altına girmez: içerik çentikten 8 pt aşağıda başlar (`NotchLayout.expandedTopGap`).
2. **Başlık:** "✳ Claude Code" tek satır. Claude çalışırken sağda proje adı + dalgalanan üç nokta (yalnızca çalışırken oynar), ✳ nabız atar; boştayken sağ taraf boş.
3. **Çipler:** ikon + sayı: `$` bugünkü harcama (fiyatı bilinmeyen model varsa "—"; tooltip hangi model), `#` bugünkü token, alev = aktif bloğun hızı ("3K/min"). ccusage yoksa tek çip "↓ ccusage" (tooltip kurulum komutu). Sayılar değişince rakamlar kayarak geçer.
4. **Modeller:** tek model → renkli nokta + ad ("Fable 5.1"); iki+ model → parçalı çubuk (kart açılırken paylara oturur) ve altında nokta + ad + yüzde. Tek modelde dolu çubuk çizilmez.
5. **Durum noktası:** yeşil = limitler taze, turuncu = 15 dk'dan eski / rate limit / ağ yok, kırmızı = kimlik sorunu; yalnızca yapılacak bir şey varsa yanında kısa metin ("Sign in with `claude`"). Sağda plan çipi ("Team"). Her öğenin tooltip'i tam cümleyi verir.
