...
- Durum makinesi: `Core/State/NotchState.swift` (closed/compact/expanded/popup), kurallar `NotchTransition` (saf, testli), zamanlama `NotchViewModel` (`@Observable`). Hover çıkışı 1,5 s gecikmeli; kartın çevresindeki tolerans bölgesi `NotchLayout.graceRect` ile geometrik hesaplanır, görünmez alan çizilmez (alfa geçirgenliğini bozar). Modül içeriği `NotchContentProvider` üzerinden gelir; sağlayıcıyı `ModuleManager.contentProvider()` üretir.
