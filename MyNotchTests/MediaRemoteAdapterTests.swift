...
final class MediaRemoteAdapterTests: XCTestCase {
    ...
    func testHealthRecordsRoundTrip() {
        ...
        let record = AdapterHealthRecord(status: .broken(exitCode: 7), osBuild: "26.6.2", appVersion: "0.1.0/1", checkedAt: Date(timeIntervalSince1970: 1_800_000_000))
        XCTAssertEqual(AdapterHealthRecord(storedValue: record.storedValue), record)
        XCTAssertTrue(record.isCurrent(osBuild: "26.6.2", appVersion: "0.1.0/1"))
        XCTAssertFalse(record.isCurrent(osBuild: "26.7", appVersion: "0.1.0/1"), "a new macOS invalidates the verdict")
