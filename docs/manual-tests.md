...
15. **Parça değişimi popup'ı:** şarkı değişince kapak + başlık + sanatçı satırı çentiğin **altındaki** şeritte, tüm genişlikte görünür; uzun başlık kameranın arkasına girmez, sonu üç nokta ile kısalır. Kontrol: `scripts/run.sh --args -debugState popup -debugModule media`. Harici (çentiksiz) ekranda aynı satır kapsülün içinde ortalı.
